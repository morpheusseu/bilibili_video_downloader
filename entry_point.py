...
def download_videos_from_given_upper_favorite_list(credential, uid, convert_type="wav", list_name_re="", progress=None, conn=None):
    uid = int(uid)
...
def download_videos_from_given_favorite_list_id(credential, fid, convert_type="wav", progress=None, conn=None):
...
def download_video_from_given_bvid(convert_type, bv_id, credential, page_idx=0, cid=None, progress=None, conn=None, static_info=None):
...
def download_videos_from_given_link_adjustment(web_link, convert_type, credential, progress=None, conn=None):
    progress = Progress(
        SpinnerColumn(), '{task.description}', BarColumn(), MofNCompleteColumn(
        ), TaskProgressColumn(), TimeElapsedColumn(), TimeRemainingColumn()
    ) if progress else None
    ...
    for item in parts:
        ...
        if item.startswith("favlist?fid="):
            ...
            download_videos_from_given_favorite_list_id(
                credential=credential, fid=fid, convert_type=convert_type, progress=progress, conn=conn)
            break
        if video_flag:
            if item.startswith('BV'):
                ...
                download_video_from_given_bvid(
                    convert_type=convert_type, bv_id=bvid, credential=credential, progress=progress, conn=conn)
            break
        if item == 'video':
            video_flag = True
    progress.stop() if progress else None
